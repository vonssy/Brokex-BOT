...
class Brokex:
    ...
    def print_leverage_question(self):
        while True:
            try:
                leverage = int(input(f"{Fore.YELLOW + Style.BRIGHT}Enter Leverage [1-50] -> {Style.RESET_ALL}").strip())
                if 1 <= leverage <= 50:
                    self.leverage = leverage
                    print(f"{Fore.GREEN + Style.BRIGHT}Leverage {leverage}x Selected.{Style.RESET_ALL}")
                    break
                else:
                    print(f"{Fore.RED + Style.BRIGHT}Leverage must be between 1 and 50.{Style.RESET_ALL}")
            except ValueError:
                print(f"{Fore.RED + Style.BRIGHT}Invalid input. Enter an integer.{Style.RESET_ALL}")
    ...
    async def process_perform_open_potition(self, account: str, address: str, pair: int, is_long: bool, use_proxy: bool):
        tx_hash, block_number = await self.perform_open_potition(account, address, pair, is_long, use_proxy, self.leverage)
        if tx_hash and block_number:
            explorer = f"https://testnet.pharosscan.xyz/tx/{tx_hash}"
    ...
    async def process_option_2(self, account: str, address: str, use_proxy: bool):
        ...
        for i in range(self.potition_count):
            ...
            self.log(
                ...
                f"{Fore.WHITE+Style.BRIGHT} {self.leverage}x {Style.RESET_ALL}"
            )
            self.log(
    ...
    async def main(self):
        try:
            ...
            while True:
                use_proxy_choice = int(input(f"{Fore.BLUE + Style.BRIGHT}Choose Proxy Option [1/2/3] -> {Style.RESET_ALL}").strip())
